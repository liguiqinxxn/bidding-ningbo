...
export default defineComponent({
  setup() {
    ...
    const sidebarclick = (item: any) => {
      ...
      state.heavy = item.heavy;
      $router.push({
        query: ($route.query, { type: item.type, heavy: item.heavy }),
      });
      state.isShow = false;
      state.keyword = "";
    };

    const modelListFn = async (type: any, heavy: any) => {
      const params = {
        type,
        heavy,
    ...
    const init = () => {
      modelListFn(state.type, state.heavy).then((res) => {
        state.list = res.data || [];
    ...
    const openDetails = (item: any) => {
      ...
      const to = $router.resolve({
        path: $route.path,
        query: ($route.query, { type: state.type, heavy: state.heavy, articleId: item.id }),
      });
